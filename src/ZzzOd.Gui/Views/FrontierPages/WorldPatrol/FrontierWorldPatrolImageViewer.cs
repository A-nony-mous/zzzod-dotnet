...
internal sealed record FrontierWorldPatrolImageViewerOption(string Label, string Value)
{
    public override string ToString() => Label;
}

internal sealed class FrontierWorldPatrolImagePointEventArgs(
    int x,
    int y,
    double displayX,
    double displayY,
...
internal sealed class FrontierWorldPatrolImageAreaEventArgs(int x, int y, int width, int height) : EventArgs
{
    public int X { get; } = x;
    public int Y { get; } = y;
...
internal sealed partial class FrontierWorldPatrolImageViewer : UserControl
{
    ...
    public FrontierWorldPatrolImageViewer()
    {
        ...
        {
            new FrontierWorldPatrolImageViewerOption("点击模式", "click"),
            new FrontierWorldPatrolImageViewerOption("框选模式", "select"),
        };
        _modeCombo.SelectedIndex = 0;
    }

    public event EventHandler<FrontierWorldPatrolImagePointEventArgs>? PointClicked;

    public event EventHandler<FrontierWorldPatrolImageAreaEventArgs>? AreaSelected;
    ...
    private bool IsSelectMode => _modeCombo.SelectedItem is FrontierWorldPatrolImageViewerOption { Value: "select" };
    ...
    private void OnImagePointerPressed(object? sender, PointerPressedEventArgs args)
    {
        ...
        PointClicked?.Invoke(this, new FrontierWorldPatrolImagePointEventArgs(
            (int)original.X,
            (int)original.Y,
            display.X,
            display.Y,
    ...
    private void OnImagePointerReleased(object? sender, PointerReleasedEventArgs args)
    {
        ...
        AreaSelected?.Invoke(this, new FrontierWorldPatrolImageAreaEventArgs(left, top, width, height));
        UpdateSelectionRectangle();
    }
