...
public static class AgentStateChecker
{
	...
	public static int CheckTemplateFound(Mat source, Mat template, double threshold = 0.8, Mat? mask = null)
	{
		...
		else
		{
			...
			// 带掩码的 TM_CCOEFF_NORMED 可能产生 NaN；阈值比较对 NaN 为假、对正无穷为真，
			// 因此先替换 NaN 再取最大值。
	...
	private static Mat Dilate(Mat mask, int kernelSize)
	{
		if (kernelSize == 0)
		{
			...
			// 使用 k×k 全一核并只迭代一次，不折算为 3×3 多次迭代。
		Mat mat = new Mat();
	...
	private static AgentStateDef ResolveKnownColorAgentState(AgentStateDef stateDef, int? total, int? pos)
	{
		...
		AgentStateDef result = stateName switch
		{
			...
			"叶瞬光-青溟剑势-白" => new AgentStateDef(stateDef.StateName, AgentStateCheckWay.COLOR_RANGE_CONNECT, "yeshunguang_qingming_ex", null, null, new int[3] { 0, 0, 255 }, new int[3] { 0, 0, 0 }, 10),
			...
			"蕾米埃尔-浮晖" => new AgentStateDef(stateDef.StateName, AgentStateCheckWay.FOREGROUND_COLOR_RANGE_LENGTH, "remielle_floating_radiance", null, null, new int[3] { 150, 255, 255 }, new int[3] { 30, 255, 200 }, null, null, 100),
			"蕾米埃尔-虚曜" => new AgentStateDef(stateDef.StateName, AgentStateCheckWay.COLOR_RANGE_CONNECT, "remielle_voidflare", null, null, new int[3] { 150, 255, 255 }, new int[3] { 30, 255, 200 }, 1),
			_ => stateDef,
		};
		if (1 == 0)
		{
