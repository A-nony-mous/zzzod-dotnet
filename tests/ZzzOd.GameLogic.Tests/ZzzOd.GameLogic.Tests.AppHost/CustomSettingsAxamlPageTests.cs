...
public sealed class CustomSettingsAxamlPageTests
{
	...
	public void CustomSettingsUsesAxamlFluentControlsAndRealServices()
	{
		...
		Assert.Contains("<ComboBox x:Name=\"LanguageCombo\"", text, StringComparison.Ordinal);
		Assert.Contains("ItemsSource=\"{Binding LanguageOptions}\"", text, StringComparison.Ordinal);
		...
		string viewModelString = File.ReadAllText(Path.Combine(path, "ZzzCustomSettingsViewModel.cs"));
		Assert.Contains("SaveConfigScope", viewModelString, StringComparison.Ordinal);
		...
		Assert.Contains("RequestedThemeVariant", viewModelString, StringComparison.Ordinal);
		...
		Assert.Contains("SelectedShellPresetValue", viewModelString, StringComparison.Ordinal);
		...
		Assert.Contains("PersistCustomBanner", viewModelString, StringComparison.Ordinal);
		Assert.DoesNotContain("[\"custom_banner\"] = true", viewModelString, StringComparison.Ordinal);
